...
const QuizPage = ({
  ...
  const headerBadge = useMemo(() => {
    if (!quiz) return null;
    return quiz.quiz_state === "locked" ? <span className="quiz__badge quiz__badge--locked">Verrouillé</span> : <span className="quiz__badge quiz__badge--unlocked">En édition</span>;
  }, [quiz]);
  ...
  return (
    <div className="quiz">
      <header className="quiz__hero">
        <div className="quiz__hero-content">
          <div className="quiz__hero-headline">
            <span className="quiz__eyebrow">Radi Quiz</span>
            <div className="quiz__title-row">
              <h1>{form.quiz_title || "Sans titre"}</h1>
              {headerBadge}
            </div>
            <p>
              Gérez les informations générales de votre quiz. Ces paramètres alimentent AMC et les communications envoyées aux étudiants.
            </p>
          </div>
          <div className="quiz__hero-actions">
            <button className="quiz__secondary" onClick={onBack}>
              Retour au tableau de bord
            </button>
            <button className="quiz__secondary" onClick={onNavigateSettings}>
              Paramètres
            </button>
            <button className="quiz__secondary" onClick={onNavigateHelp}>
              Centre d'aide
            </button>
            <button className="quiz__secondary" onClick={onLogout}>
              Déconnexion
            </button>
          </div>
        </div>
        <nav className="quiz__tabs" aria-label="Navigation Quiz">
          <button className="quiz__tab quiz__tab--active">Généralités</button>
          <button className="quiz__tab" disabled>
            Questions
          </button>
          <button className="quiz__tab" disabled>
            Sujet
        ...
        <section className="quiz__section">
          <form className="quiz__form" onSubmit={handleSave}>
            ...
            <div className="quiz__actions">
              <button className="quiz__primary" type="submit" disabled={saving}>
                Enregistrer
              </button>
            </div>
            {message ? <div className="quiz__status">{message}</div> : null}
            {error ? <div className="quiz__error">{error}</div> : null}
            {isLocked ? (
